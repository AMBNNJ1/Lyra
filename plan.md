...
- **Goal:** Ensure the Python/Node apps share consistent configuration.
  - Copy `.env.example` to `.env`, then populate Clerk, Mem0, Kokoro, and search API keys.
...
- **Goal:** Load starting persona/user context so conversations feel grounded.
  - Use `tools/init_memory_db.py` (and/or `.env` values) to write persona + user label into Mem0.
...
- **Goal:** Ensure important facts persist and retrieval packs are rich.
  ...
  - Inspect memory (via dashboard CLI or `tools/memory_dashboard.py`) to confirm labels (`profile`, `preferences`, `facts`, `goals`).
...
- **Goal:** Prepare for public MVP launch.
  ...
  - Re-point Mem0 credentials to production instances and rerun quick smoke tests.
